  ...
  const filteredProgrammers = programmersData
    .filter((programmer) => {
      const matchesSearch =
        ...
        programmer.role.toLowerCase().includes(searchTerm.toLowerCase()) ||
        programmer.stack.some((tech) => tech.toLowerCase().includes(searchTerm.toLowerCase()))

      const matchesCategory = selectedCategory === "all" || programmer.category === selectedCategory
      ...
      <section className="py-16 px-4 bg-gradient-to-br from-background via-muted/30 to-background">
        <div className="container mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-5xl font-bold mb-4">Programmers Directory</h1>
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
              Discover talented developers from around the world. Connect, collaborate, and learn from each other.
            </p>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="text-center text-muted-foreground mb-8">
              Showing {filteredProgrammers.length} programmer{filteredProgrammers.length !== 1 ? "s" : ""}
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProgrammers.map((programmer) => (
              <Card key={programmer.id} className="hover:shadow-lg transition-all duration-300 hover:scale-105">
                <CardHeader className="text-center">
                  ...
                  />
                  <CardTitle className="text-xl">{programmer.name}</CardTitle>
                  <CardDescription className="text-lg font-medium text-secondary">{programmer.role}</CardDescription>
                ...
                <CardContent className="space-y-4">
                  <p className="text-sm text-muted-foreground text-center">{programmer.bio}</p>
                  ...
                  <div className="flex justify-center space-x-6 text-sm">
                    ...
                    <div className="text-muted-foreground">{programmer.projects} projects</div>
                  </div>

                  {/* Tech Stack */}
                  <div className="flex flex-wrap gap-2 justify-center">
                    {programmer.stack.slice(0, 4).map((tech) => (
                    ...
                    {programmer.stack.length > 4 && (
                      <Badge variant="outline" className="text-xs">
                        +{programmer.stack.length - 4} more
                      </Badge>
                    )}
                  </div>
                  ...
                  <Button className="w-full" asChild>
                    <Link href={`/programmers/${programmer.id}`}>View Profile</Link>
                  </Button>
                </CardContent>
              </Card>
            ))}
          ...
          {filteredProgrammers.length === 0 && (
            <div className="text-center py-16">
              <h3 className="text-2xl font-semibold mb-4">No programmers found</h3>
              <p className="text-muted-foreground mb-8">Try adjusting your search criteria or filters.</p>
              <Button
                variant="outline"
                onClick={() => {
                  setSearchTerm("")
      ...
      <section className="py-16 px-4 bg-muted/30">
        <div className="container mx-auto text-center">
          ...
          <p className="text-xl text-muted-foreground mb-8">
            Join our community and showcase your skills to thousands of developers.
          </p>
          <Button size="lg" asChild>
            <Link href="/register">Join Paginate.dev</Link>
          </Button>
        </div>
      </section>
