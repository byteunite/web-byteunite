...
/**
 * API Endpoint untuk capture screenshot sites menggunakan Puppeteer
 *
 * Approach:
...
export async function POST(request: NextRequest) {
    try {
        const { siteId, totalSlides } = await request.json();

        if (!siteId || !totalSlides) {
            return NextResponse.json(
                { error: "siteId and totalSlides are required" },
                { status: 400 }
            );
        }
        ...
        const url = `${baseUrl}/template/${siteId}?data=sites&screenshot=true`;
